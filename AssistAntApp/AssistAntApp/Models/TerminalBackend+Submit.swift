...
/// Prompt submission for text this app composed itself — Quick Capture,
/// scheduled tasks, Send to Claude, /clear, /compact. Kept in the app (not
/// Galactic) so the gesture needs no engine release: it rides existing
/// public protocol requirements. Mirrors the companion app's seam of the same
/// name; the two must agree, because they write one shared keybindings file.
///
...
enum SessionSubmit {
    ...
    static var bytes: [UInt8] {
        ...
        // and can be edited or deleted while this app runs. It reads a few
    ...
    /// lines are what finally distinguished "the app sent the wrong thing" from
    /// "the app sent the right thing into a terminal that could not yet decode
    ...
    ///
    ///     tail -f ~/.assist-ant/assist-ant-app.log | grep AssistAnt/submit
    static func log(_ message: String) {
        AssistAntLog.submit(message)
    }
...
extension TerminalBackend {
    ...
    func submitPrompt() {
        ...
        // `AgentSessionController.sendCommand`. Do not drop that gate on the
        // belief that this wait already covers it — it does not, and the
