...
final class ActionableListChords {
    ...
    private func applyChord(leader: Character, key: String?, _ ctx: Context) -> Bool {
        ...
        case ("a", "p") where ctx.tab == .trash:
            _ = ctx.actions.putBack(selected.filter { !$0.isSynced })   // skip synced
        ...
        case ("l", "p") where ctx.tab != .trash:
            _ = ctx.actions.putBack(selected.filter { !$0.isSynced })   // put back held rows
