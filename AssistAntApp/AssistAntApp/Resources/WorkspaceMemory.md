...
derives to fill the column. Park one yourself with `assist-ant scratch add` —
`--list NAME` parks it already grouped, in the same list namespace the
actionable items use — and read the feed with `assist-ant scratch list` (JSON;
ids and any list included) when you need to find a note to talk about or
convert.
...
rewritten in place, keeping its id, the time it was parked, and whatever list it
was parked in. The Scratch tab's convert gesture writes a payload of one or more
notes plus the chosen kind and invokes the skill for you; reach for it directly
when the user asks to make something of a note they parked. Each note is
converted by its own CLI call, so one bad note never strands the rest, and
nothing in the app reports on the run — a converted note leaves the feed and a
failure is visible only in what you say, so name every note you did and did not
convert.
